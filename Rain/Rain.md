...
我们思考一个问题，高度遮挡该怎么做？用雨丝的高度和场景物体的高度做遮挡测试，剔除掉那些被遮挡的雨丝。这样我们就面临两个问题：
+ 如何计算雨丝的高度
+ 如何计算场景的高度

雨丝的高度计算和它的深度有相同的问题：在纹理动画中模拟出来的雨丝，它的实际位置都是在梭体内表面上的，并不能把这个位置当做每个雨丝的位置。在前面我们已经知道了雨丝的深度信息（距离相机的位置），那么再结合相机自身的信息，能算出雨丝完整的世界空间坐标么？

显然是可以的

首先，我们从相机中看到了某个像素点 $(u, v)$ 上的雨丝，我们知道这个雨丝的深度值为 $rainDepth$ 。

假设雨丝的世界空间坐标为 $(x, y, z)$ ，这个点经过 MVP 变换和裁剪后最终会投射到屏幕空间的像素点 $(u, v)$ 上。

三维空间中的场景被投影到了二维的屏幕上，那么像素点 $(u, v)$ 对于三维空间中的场景意味着什么，意味着从像素点出发沿着相机的 $lookat$ 方向发射的一条射线，距离相机最近的物体会被投影到像素点上。
~~【TODO：图片或者作图演示 展示从像素点发射一条射线】~~

所以，我们先对像素点 $(u, v)$ 做逆变换得到这条射线，雨丝一定在这条射线上，直接带入雨丝的深度 $rainDepth$ 就可以计算出雨丝的世界空间坐标。

高度图的生成较为简单，它跟相机视角场景深度图以及阴影贴图的生成流程完全一样：
+ 设定相机上方 $height$ 米的位置作为高度图的初始位置
    + 这里不使用相机正上方作为初始位置，因为在正上方生成的高度图有大块的无效区域，即相机看不到的区域，我们不关注这些地方有没有下雨，毕竟我们也看不到下雨
    ~~【TODO：图片或者作图演示 选正上方的缺点】~~
    + 相机正前方某一点，并竖直平移 $height$ 的高度，选取这点作为初始位置$(lookat.x * size, height, look.z * size)$ ，最终要保证相机在投影区域的边缘并垂直的看向投影区域的中心
    ~~【TODO：图片或者作图演示 展示正确的初始位置】~~
+ 在初始位置使用正交投影固定方向的向下观察
+ 记录下每一个点离相机最近物体的高度值（这里就是物体世界坐标的 $Y$ 值）
+ 最终得到了相机正前方区域的高度图

最终，我们使用雨丝世界空间坐标的 $Y$ 值和该点采样得到的高度，就可以进行遮挡判断了
